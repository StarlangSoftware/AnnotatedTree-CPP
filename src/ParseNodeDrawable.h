//
// Created by Olcay Taner YILDIZ on 2019-05-12.
//

#ifndef ANNOTATEDTREE_PARSENODEDRAWABLE_H
#define ANNOTATEDTREE_PARSENODEDRAWABLE_H


#include <ParseNode.h>
#include "LayerInfo.h"
#include "ParseTreeSearchable.h"
#include "ParseNodeSearchable.h"

class ParseNodeDrawable : public ParseNode{
protected:
    LayerInfo* layers = nullptr;
    int depth;
public:
    ParseNodeDrawable(ParseNodeDrawable* parent, const string& line, bool isLeaf, int depth);
    explicit ParseNodeDrawable(const Symbol& data);
    ParseNodeDrawable(ParseNodeDrawable* parent, ParseNodeDrawable* child, const string& symbol);
    [[nodiscard]] LayerInfo* getLayerInfo() const;
    [[nodiscard]] Symbol getData() const;
    void clearLayers();
    void clearLayer(ViewLayerType layerType) const;
    void clearData();
    void setDataAndClearLayers(const Symbol& data);
    void setData(const Symbol& data);
    [[nodiscard]] string headWord(ViewLayerType viewLayerType) const;
    [[nodiscard]] string getLayerData() const;
    [[nodiscard]] string getLayerData(ViewLayerType viewLayer) const;
    [[nodiscard]] int getDepth() const;
    int structureAgreementCount(const ParseNodeDrawable* parseNode) const;
    int glossAgreementCount(const ParseNodeDrawable* parseNode, ViewLayerType viewLayerType) const;
    void updateDepths(int _depth);
    [[nodiscard]] int maxDepth() const;
    [[nodiscard]] string ancestorString() const;
    [[nodiscard]] bool layerExists(ViewLayerType viewLayerType) const;
    [[nodiscard]] bool isDummyNode() const;
    [[nodiscard]] bool layerAll(ViewLayerType viewLayerType) const;
    [[nodiscard]] string toTurkishSentence() const;
    void checkGazetteer(const Gazetteer& gazetteer, const string& word) const;
    void generateParseNode(ParseNode* parseNode, bool surfaceForm) const;
    [[nodiscard]] string to_string() const;
    bool satisfy(const ParseNodeSearchable* node) const;
    vector<ParseNodeDrawable*> satisfy(const ParseTreeSearchable& tree);
};


#endif //ANNOTATEDTREE_PARSENODEDRAWABLE_H
